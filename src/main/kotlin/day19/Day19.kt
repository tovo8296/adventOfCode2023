...
enum class Property(val text: String) {
    Cool("x"),
    Music("m"),
    Aerodynamic("a"),
    Shiny("s");
...
data class Part(val cool: Int, val music: Int, val aerodynamic: Int, val shiny: Int) {
    fun get(prop: Property): Int = when (prop) {
        Property.Cool -> cool
        Property.Music -> music
        Property.Aerodynamic -> aerodynamic
        Property.Shiny -> shiny
    }
}

data class Workflow(val name: String, val rules: List<Rule>)
data class Rule(val property: Property, val greater: Boolean, val value: Int, val nextWorkflow: String) {
    fun matches(part: Part): Boolean {
        val actualGreater = part.get(property) > value
        return actualGreater == greater
    }
}
...
fun main() {
    ...
    val accepted = parts.filter { followWorkflows(it, "in", workflows) == "A" }
    val sum = accepted.map {
        it.cool + it.aerodynamic + it.music + it.shiny
    }.sum()
    println("Sum: $sum")
}

fun followWorkflows(part: Part, startRule: String, workflows: Map<String, Workflow>): String {
    var rule = startRule
    while (rule != "A" && rule != "R") {
        val workflow = workflows[rule]!!
        val matchingRule = workflow.rules.find { it.matches(part) }!!
        rule = matchingRule.nextWorkflow
    }
    return rule
}

fun parseWorkflows(): Map<String, Workflow> =
    input.lines().map {
        val match = "([a-z]+)\\{(.*)\\}".toRegex().matchEntire(it)!!
        val name = match.groups[1]!!.value
        val rules = match.groups[2]!!.value.split(",").map { parseRule(it) }
        Workflow(name, rules)
    }.associateBy { it.name }

fun parseRule(text: String): Rule {
    return if (!text.contains("<") && !text.contains(">")) {
        Rule(Property.Cool, true, -1, text)
    } else {
        val match = "([a-z])([<>])(\\d+):([a-zAR]+)".toRegex().matchEntire(text)!!
        val prop = Property.entries.find { it.text == match.groups[1]!!.value }!!
        val greater = match.groups[2]!!.value == ">"
        val number = match.groups[3]!!.value.toInt()
        val followup = match.groups[4]!!.value
        Rule(prop, greater, number, followup)
    }
}

fun parseParts(): List<Part> = partInput.lines().map {
    val groups = "\\{x=(\\d+),m=(\\d+),a=(\\d+),s=(\\d+)\\}".toRegex().matchEntire(it)!!.groups
    Part(
