...
fun main() {
    ...
    val sum2 = games.sumOf { game ->
        val red = game.handfulls.map { it.red }.max()
        val green = game.handfulls.map { it.green }.max()
        val blue = game.handfulls.map { it.blue }.max()
        red * green * blue
    }
    println("Part 2: $sum2")
...
private fun parseLine(line: String): Game {
    val id = "Game (\\d+):".toRegex().find(line)!!.groupValues[1].toInt()
    val handfulTexts = line.substringAfter(":").split(";")
    val handfuls = handfulTexts.map { parseHandful(it) }
    return Game(id, handfuls)
}

private fun parseHandful(text: String): Handfull {
    val green = parseColor(text, "green")
    val blue = parseColor(text, "blue")
    val red = parseColor(text, "red")
    return Handfull(red, green, blue)
}

private fun parseColor(text: String, color: String) =
    " (\\d+) $color".toRegex().find(text)?.groupValues?.get(1)?.toInt() ?: 0
