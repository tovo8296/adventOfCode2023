...
fun main() {
    val numbers = parse()
    val sum = numbers
        .filter { zahl -> hasAdjacentSymbol(zahl) }
        .map { zahl -> zahl.value }
        .sum()
    println("Part numbers sum: $sum")
    ...
    input.lines().forEachIndexed { y, line ->
        line.forEachIndexed { x, c ->
            if (c == '*') {
                val adjNumbers = findAdjacentNumbers(Coord(x, y), numbers)
                if (adjNumbers.size == 2) {
                    calibration += adjNumbers
                        .map { it.value }
                        .fold(1) { acc, n -> acc * n }
                }
...
fun findAdjacentNumbers(coord: Coord, numbers: MutableList<PartNumber>): List<PartNumber> =
    numbers.filter {
        getAdjacentCoords(it).contains(coord)
    }


fun getAdjacentCoords(number: PartNumber): List<Coord> {
    val coords = mutableListOf<Coord>()
    for (x in number.start - 1..number.end + 1) {
        coords.add(Coord(x, number.lineNumber - 1))
        coords.add(Coord(x, number.lineNumber + 1))
    }
    coords.add(Coord(number.start - 1, number.lineNumber))
    coords.add(Coord(number.end + 1, number.lineNumber))
    return coords
}

fun hasAdjacentSymbol(number: PartNumber): Boolean =
    getAdjacentCoords(number).any {
        isSymbol(it)
    }

fun isSymbol(coord: Coord): Boolean {
    val c = input.lines().getOrNull(coord.y)?.getOrNull(coord.x)
    return c != null && c != '.' && !c.isDigit()
}

private fun parse(): MutableList<PartNumber> {
    val numbers = mutableListOf<PartNumber>()
    input.lines()
        .mapIndexed { zeilennummer, line ->
        parseLine(line, zeilennummer)
    }.forEach {
        numbers.addAll(it)
    }
    return numbers
...
private fun parseLine(line: String, lineNumber: Int): List<PartNumber> {
    var searchPos = 0
    val result = mutableListOf<PartNumber>()
    while (searchPos < line.length) {
        val match = "\\d+".toRegex().find(line, searchPos)
        if (match != null) {
            val number = PartNumber(
                value = line.substring(match.range).toInt(),
                lineNumber = lineNumber,
                start = match.range.start,
                end = match.range.endInclusive
            )
            result.add(number)
            searchPos = match.range.endInclusive + 1
        } else {
            break
        }
    }
